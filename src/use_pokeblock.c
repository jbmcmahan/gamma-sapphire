...
{
    [CONDITION_COOL]   = MON_DATA_SPATK_EV,
    [CONDITION_TOUGH]  = MON_DATA_SPDEF_EV,
    [CONDITION_SMART]  = MON_DATA_ATK_EV,
    [CONDITION_CUTE]   = MON_DATA_DEF_EV,
    [CONDITION_BEAUTY] = MON_DATA_SPEED_EV
};
...
static void AddPokeblockToConditions(struct Pokeblock *pokeblock, struct Pokemon *mon)
{
    ...
    if (GetMonData(mon, MON_DATA_HP_EV) != MAX_SHEEN)
    {
        ...
        stat = (u8)(GetMonData(mon, MON_DATA_HP_EV)) + pokeblock->feel;
        if (stat > MAX_SHEEN)
            stat = MAX_SHEEN;

        data = stat;
        SetMonData(mon, MON_DATA_HP_EV, &data);
    }
}
...
{
    u8 sheen = GetMonData(&gPlayerParty[monIndex], MON_DATA_HP_EV);
