...
{
    return !IsTerastallized(battlerId);
}
...
void TeraIndicator_SetVisibilities(u32 healthboxId, bool32 invisible)
{
    ...
    if (invisible == TRUE)
        ...
        gSprites[spriteId].invisible = TeraIndicator_ShouldBeInvisible(battlerId);
}
...
void TeraIndicator_CreateSprite(u32 battlerId, u32 healthboxSpriteId)
{
    ...
    s32 x = 0;
    u32 type = GetTeraType(battlerId);
    ...
    spriteId = gBattleStruct->tera.indicatorSpriteId[battlerId] = CreateSpriteAtEnd(sTeraIndicatorSpriteTemplates[type], 0, y, 0);
...
void TeraIndicator_UpdateType(u32 battlerId, u32 healthboxSpriteId)
{
    ...
    TeraIndicator_CreateSprite(battlerId, healthboxSpriteId);
}
