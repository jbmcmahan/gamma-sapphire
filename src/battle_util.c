    ...
    do
    {
        ...
        case CANCELLER_POWDER_MOVE:
            if ((gBattleMoves[gCurrentMove].flags & FLAG_POWDER) && (gBattlerAttacker != gBattlerTarget))
            {
                ...
                if (IsBattlerOfType(gBattlerTarget, TYPE_GRASS) || BattlerHasAbilityOrInnate(gBattlerTarget, ABILITY_OVERCOAT))
            #else
...
u8 AbilityBattleEffects(u8 caseID, u8 battler, u32 ability, u8 special, u16 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_MOLD_BREAKER:
            if (!gSpecialStatuses[battler].switchInAbilityOrInnateDone[GetBattlerAbilityOrInnateNum(battler, battlerAbility)])
            {
        ...
        case  ABILITY_DRIZZLE:
            if (TryChangeBattleWeather(battler, ENUM_WEATHER_RAIN, TRUE))
            {
             ...
             && !gSpecialStatuses[battler].switchInAbilityOrInnateDone[GetBattlerAbilityOrInnateNum(battler, battlerAbility)])
            {
                gSpecialStatuses[battler].switchInAbilityOrInnateDone[GetBattlerAbilityOrInnateNum(battler, battlerAbility)] = TRUE;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_MAGICAL_DUST:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && IsMoveMakingContact(move, gBattlerAttacker)
             && TARGET_TURN_DAMAGED
             && !IsBattlerOfType(gBattlerAttacker, TYPE_PSYCHIC))
            {
...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
	...
	if(BattlerHasAbilityOrInnate(battlerAtk, ABILITY_DRAGONSLAYER)){
	    if (IsBattlerOfType(battlerDef, TYPE_DRAGON)) // check if foe has Dragon-type
            MulModifier(&modifier, UQ_4_12(1.5));
	}
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    if(BattlerHasAbilityOrInnate(battlerAtk, ABILITY_SEAWEED))
		if (moveType == TYPE_GRASS && IsBattlerOfType(battlerDef, TYPE_FIRE))
        {
            MulModifier(&modifier, UQ_4_12(2.0));
            if (updateFlags)
	...
	if(BattlerHasAbilityOrInnate(battlerDef, ABILITY_SEAWEED)){
        if (moveType == TYPE_FIRE && IsBattlerOfType(battlerDef, TYPE_GRASS))
        {
            MulModifier(&modifier, UQ_4_12(0.5));
            if (updateFlags)
                RecordAbilityBattle(battlerDef, ABILITY_SEAWEED);
    ...
    if (!IsBattlerGrounded(battlerDef) && 
       ...
       moveType == TYPE_GROUND){
        if(gBattleMons[battlerDef].type1 == TYPE_FLYING && gBattleMons[battlerDef].type2 != TYPE_FLYING){
            ...
            //Everything else
            if(IsBattlerOfType(battlerDef, TYPE_ELECTRIC) ||
               IsBattlerOfType(battlerDef, TYPE_FIRE)     ||
               IsBattlerOfType(battlerDef, TYPE_POISON)   ||
               IsBattlerOfType(battlerDef, TYPE_ROCK)     ||
               IsBattlerOfType(battlerDef, TYPE_STEEL)){
