...
#define PSS_LABEL_WINDOW_POKEMON_ABILITIES_TITLE 2
...
// Abilities screen
#define PSS_LABEL_WINDOW_POKEMON_ABILITY_1 11
#define PSS_LABEL_WINDOW_POKEMON_ABILITY_2 12
#define PSS_LABEL_WINDOW_POKEMON_ABILITY_3 13
#define PSS_LABEL_WINDOW_POKEMON_ABILITY_4 14

// Skills screen
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_LEFT 15 // HP, Attack, Defense
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_RIGHT 16 // Sp. Attack, Sp. Defense, Speed
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_EXP 17 // EXP, Next Level
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_STATUS 18
...
// Dynamic fields for the Pokemon Abilities page
#define PSS_DATA_WINDOW_ABILITY 0
#define PSS_DATA_WINDOW_INNATE1 1
#define PSS_DATA_WINDOW_INNATE2 2
#define PSS_DATA_WINDOW_INNATE3 3
...
static void PrintMonInnateName(u8 innateNum);
static void PrintMonInnateDescription(u8 innateNum);
...
static void Task_HandleInput(u8 taskId)
{
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        else if (JOY_NEW(A_BUTTON))
        {
            ...
            else if (sMonSummaryScreen->currPageIndex == PSS_PAGE_ABILITIES)
            {
                PlaySE(SE_SELECT);
            ...
            {
                PlaySE(SE_SELECT);
                // SwitchToSkillsSelection(taskId);
            }
            else
            {
                PlaySE(SE_SELECT);
...
static void Task_ShowPowerAccWindow(u8 taskId)
{
    ...
    if (data[1] <= 0 || data[1] >= sBattleMoveTilemapCtrl.field_6)
    {
        if (data[0] < 0)
        {
            if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES || sMonSummaryScreen->currPageIndex == PSS_PAGE_ABILITIES)
...
static void PrintMonAbilityName(void)
{
    ...
    PrintSmallTextOnWindow(AddWindowFromTemplateList(sPageAbilitiesTemplate, PSS_DATA_WINDOW_ABILITY), gAbilityNames[ability], 70, 4, 0, 1);
}

static void PrintMonAbilityDescription(void)
{
    u16 ability = GetAbilityBySpecies(sMonSummaryScreen->summary.species, sMonSummaryScreen->summary.abilityNum);
    PrintSmallTextOnWindow(AddWindowFromTemplateList(sPageAbilitiesTemplate, PSS_DATA_WINDOW_ABILITY), gAbilityDescriptionPointers[ability], 0, 15, 0, 0);
}

// Innate Name and Description
static void PrintMonInnateName(u8 innateNum)
{
    u16 ability = gSpeciesInfo[sMonSummaryScreen->summary.species].innates[innateNum];
    PrintSmallTextOnWindow(AddWindowFromTemplateList(sPageAbilitiesTemplate, PSS_DATA_WINDOW_INNATE1 + innateNum), gAbilityNames[ability], 70, 4, 0, 1);
...
{
    u16 ability = gSpeciesInfo[sMonSummaryScreen->summary.species].innates[innateNum];
    PrintSmallTextOnWindow(AddWindowFromTemplateList(sPageAbilitiesTemplate, PSS_DATA_WINDOW_INNATE1 + innateNum), gAbilityDescriptionPointers[ability], 0, 15, 0, 0);
}
...
{
    PrintMonAbilityName();
    PrintMonInnateName(0);
    PrintMonInnateName(1);
    PrintMonInnateName(2);

    PrintMonAbilityDescription();
    PrintMonInnateDescription(0);
    PrintMonInnateDescription(1);
    PrintMonInnateDescription(2);
...
static void Task_PrintBattleMoves(u8 taskId)
{
    ...
    case 7:
        ...
        break;
    case 8:
        DestroySpriteAndFreeResources(sMonSummaryScreen->markingsSprite);
        break;
    case 9:
        ClearWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_DEX_NUMBER);
        break;
...
static void PrintMoveNameAndPP(u8 moveIndex)
{
    ...
    {
        pp = CalculatePPWithBonus(move, summary->ppBonuses, moveIndex);
        PrintSmallTextOnWindow(moveNameWindowId, gMoveNames[move], (moveIndex%2)*63 + 5, (moveIndex/2) * 12, 0, 1);
        ConvertIntToDecimalStringN(gStringVar1, summary->pp[moveIndex], STR_CONV_MODE_RIGHT_ALIGN, 2);
        ConvertIntToDecimalStringN(gStringVar2, pp, STR_CONV_MODE_RIGHT_ALIGN, 2);
        DynamicPlaceholderTextUtil_Reset();
        DynamicPlaceholderTextUtil_SetPlaceholderPtr(0, gStringVar1);
    ...
    {
        PrintTextOnWindow(moveNameWindowId, gText_OneDash, (moveIndex%2)*63 + 5, (moveIndex/2) * 12, 0, 1);
        text = gText_TwoDashes;
        ppState = 12;
        x = GetStringCenterAlignXOffset(FONT_NORMAL, text, 44);
    }

    // PrintSmallTextOnWindow(ppValueWindowId, text, x, moveIndex * 16 + 1, 0, ppState);
}
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            ...
            if (moveEffect != EFFECT_PLACEHOLDER) {
                ...
                PrintSmallTextOnWindow(windowId, gNotDoneYetDescription, 3, 0, 0, 0);
        }
        else
        {
            PrintTextOnWindow(windowId, gContestEffectDescriptionPointers[gContestMoves[move].effect], 6, 1, 0, 0);
...
static void HidePageSpecificSprites(void)
{
    ...
    u8 i;
    for (i = SPRITE_ARR_ID_MON+3; i < ARRAY_COUNT(sMonSummaryScreen->spriteIds); i++)
    {
...
static void SetMoveTypeIcons(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (summary->moves[i] != MOVE_NONE)
            SetTypeSpritePosAndPal(gBattleMoves[summary->moves[i]].type, 25, 96 + (i * 16), i + SPRITE_ARR_ID_TYPE);
        else
...
static void CreateMoveSelectorSprites(u8 idArrayStart)
{
    ...
    if (sMonSummaryScreen->currPageIndex >= PSS_PAGE_BATTLE_MOVES)
    {
        ...
        {
            spriteIds[i] = CreateSprite(&sMoveSelectorSpriteTemplate, i * 16 + 9, 42, subpriority);
            if (i == 0)
...
static void SpriteCB_MoveSelector(struct Sprite *sprite)
{
    ...
    if (sprite->data[0] == SPRITE_ARR_ID_MOVE_SELECTOR1){
        sprite->y2 = (sMonSummaryScreen->firstMoveIndex / 2) * 12;
        sprite->x2 = (sMonSummaryScreen->firstMoveIndex % 2) * 63;
    }
    else
...
{
    u8 i;
    u8 typesList[NUMBER_OF_MON_TYPES];
    u8 arrayPoint = 0;

    for (i = 0; i < NUMBER_OF_MON_TYPES; i++)
    {
        typesList[i] = TYPE_NONE;
    }

    for (i = TYPE_NORMAL; i < NUMBER_OF_MON_TYPES; i++)
    {
        if (gTeraMoveTable[move][i].requiresTerastalization > 0)
        {
            typesList[arrayPoint] = i;
            arrayPoint++;
        }
    }
    return typesList[number];
}
