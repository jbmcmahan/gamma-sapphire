...
static void CheckIfMaxScoreEqualExpectMove(u32 battlerId, s32 target, struct ExpectedAIAction *aiAction, const char *filename)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (gBattleStruct->aiFinalScore[battlerId][target][i] > bestScore)
        {
            bestScore = gBattleStruct->aiFinalScore[battlerId][target][i];
            bestScoreId = i;
        }
    }
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (gBattleStruct->aiFinalScore[battlerId][target][i] == gBattleStruct->aiFinalScore[battlerId][target][bestScoreId]
            && !aiAction->notMove
            ...
            && !(aiAction->moveSlots & gBitTable[bestScoreId]))
        {
            Test_ExitWithResult(TEST_RESULT_FAIL, "%s:%d: EXPECT_MOVE %S has the same best score(%d) as not expected MOVE %S", filename,
                                aiAction->sourceLine, gMoveNames[moves[i]], gBattleStruct->aiFinalScore[battlerId][target][i], gMoveNames[moves[bestScoreId]]);
        }
...
void TestRunner_Battle_CheckAiMoveScores(u32 battlerId)
{
    ...
    for (i = 0; i < MAX_AI_SCORE_COMPARISION_PER_TURN; i++)
    {
        ...
        if (scoreCtx->set)
        {
            ...
            if (scoreCtx->toValue)
            {
                PrintAiMoveLog(battlerId, scoreCtx->moveSlot1, moveId1, gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot1]);
                if (!CheckComparision(gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot1], scoreCtx->value, scoreCtx->cmp))
                {
                    Test_ExitWithResult(TEST_RESULT_FAIL, "%s:%d: Unmatched SCORE_%s_VAL %S %d, got %d",
                                        filename, scoreCtx->sourceLine, sCmpToStringTable[scoreCtx->cmp], gMoveNames[moveId1], scoreCtx->value, gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot1]);
                }
            }
            else
            {
                ...
                PrintAiMoveLog(battlerId, scoreCtx->moveSlot1, moveId1, gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot1]);
                PrintAiMoveLog(battlerId, scoreCtx->moveSlot2, moveId2, gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot2]);
                if (!CheckComparision(gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot1], gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot2], scoreCtx->cmp))
                {
                    Test_ExitWithResult(TEST_RESULT_FAIL, "%s:%d: Unmatched SCORE_%s, got %S: %d, %S: %d",
                                        filename, scoreCtx->sourceLine, sCmpToStringTable[scoreCtx->cmp], gMoveNames[moveId1], gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot1], gMoveNames[moveId2], gBattleStruct->aiFinalScore[battlerId][target][scoreCtx->moveSlot2]);
                }
            }
        }
    }
